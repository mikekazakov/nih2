...
fn radiance(table: [f64; 120], turbidity: f64, albedo: f64, x: f64) -> f32 {
    ...
    result += (1.0 - albedo) * (1.0 - turbidity_rem) * sample(slice, x);

    // high albedo, low turbidity
    let idx: usize = 6 * 10 + 6 * (int_turbidity - 1);
    let slice: [f64; 6] = table[idx..idx + 6].try_into().expect("Wrong offset");
    result += (albedo) * (1.0 - turbidity_rem) * sample(slice, x);

    if int_turbidity < 10 {
        // low albedo, high turbidity
        let idx: usize = 6 * int_turbidity;
        let slice: [f64; 6] = table[idx..idx + 6].try_into().expect("Wrong offset");
        result += (1.0 - albedo) * (turbidity_rem) * sample(slice, x);

        // high albedo, high turbidity
        let idx: usize = 6 * 10 + 6 * int_turbidity;
        let slice: [f64; 6] = table[idx..idx + 6].try_into().expect("Wrong offset");
        result += (albedo) * (turbidity_rem) * sample(slice, x);
    }
    result as f32
}
...
fn distribution(table: [f64; 1080], turbidity: f64, albedo: f64, x: f64) -> [f32; 9] {
    ...
    for i in 0..9 {
        ...
        result[i] += (1.0 - albedo) * (1.0 - turbidity_rem) * sample(p, x);
    }
    ...
    for i in 0..9 {
        let p: [f64; 6] = [slice[i + 0], slice[i + 9], slice[i + 18], slice[i + 27], slice[i + 36], slice[i + 45]];
        result[i] += albedo * (1.0 - turbidity_rem) * sample(p, x);
    }
    ...
    if int_turbidity < 10 {
        ...
        for i in 0..9 {
            let p: [f64; 6] = [slice[i + 0], slice[i + 9], slice[i + 18], slice[i + 27], slice[i + 36], slice[i + 45]];
            result[i] += (1.0 - albedo) * turbidity_rem * sample(p, x);
        }
        ...
        for i in 0..9 {
            let p: [f64; 6] = [slice[i + 0], slice[i + 9], slice[i + 18], slice[i + 27], slice[i + 36], slice[i + 45]];
            result[i] += albedo * turbidity_rem * sample(p, x);
        }
    }

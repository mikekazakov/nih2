...
impl Profiler {
    ...
    pub fn new() -> Self {
        let root = Rc::new(RefCell::new(ProfileRecord::new("root")));
        Self { root: Rc::clone(&root), stack: vec![root] }
    }
    ...
    pub fn exit(&mut self, duration: f64) {
    ...
    pub fn print(&self) {
        ...
        print_records(&[Rc::clone(&self.root)], 0);
    }

    /// Reset the profiler, clearing all records and statistics.
    pub fn reset(&mut self) {
        self.root = Rc::new(RefCell::new(ProfileRecord::new("root")));
...
impl<'a> ProfileScope<'a> {
    ...
    pub fn new(label: &'a str, profiler: &'a RefCell<Profiler>) -> Self {
        profiler.borrow_mut().enter(label);
...
impl<'a> Drop for ProfileScope<'a> {
    ...
    fn drop(&mut self) {
        ...
        self.profiler.borrow_mut().exit(duration.as_secs_f64() * 1000.0);
    }
}
...
mod tests {
    ...
    fn test_profiler_new_root() {
        ...
        let root = profiler.root.borrow();
        assert_eq!(root.label, "root");
        assert_eq!(root.samples, 0);
        assert_eq!(root.average, 0.0);
        assert!(root.children.is_empty());
        assert_eq!(profiler.stack.len(), 1);
        assert!(Rc::ptr_eq(&profiler.root, &profiler.stack[0]));
    }
    ...
    fn test_profiler_enter_exit() {
        ...
        assert_eq!(profiler.stack.len(), 2);
        {
            let top = profiler.stack.last().unwrap();
    ...
    fn test_profiler_reset() {
        ...
        profiler.enter("scope1");
        profiler.exit(5.0);
        assert_eq!(profiler.root.borrow().children.len(), 1);
    ...
    fn test_profiler_nested_scopes_and_averages() {
        ...
        {
            let current = profiler.stack.last().unwrap();
    ...
    fn test_profile_scope_creation_and_stack_update() {
...

        {
            ...
            let profiler_borrow = profiler.borrow();
            assert_eq!(profiler_borrow.stack.len(), 2);
            let top_label = profiler_borrow.stack.last().unwrap().borrow().label.clone();
            assert_eq!(top_label, "test_scope");
        }

        // After _scope is dropped, stack should be back to root
        let profiler_borrow = profiler.borrow();
        assert_eq!(profiler_borrow.stack.len(), 1);
        assert_eq!(profiler_borrow.stack[0].borrow().label, "root");
    }
    ...
    fn test_profile_scope_drop_records_duration() {
        ...
        let binding = profiler.borrow();
        let root = binding.root.borrow();
        assert_eq!(root.children.len(), 1);
        let child = &root.children[0];
        let child_borrow = child.borrow();
